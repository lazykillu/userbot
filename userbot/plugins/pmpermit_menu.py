...
Credits: written by ༺Killler༻ {@Lazykilller}
"""
import asyncio
import io 
...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "No name set yet , check pinned message in @Userbotok"
PREV_REPLY_MESSAGE = {}
        ...
        async with borg.conversation(chat) as conv:
         ...
         if y == "1":
             ...
             if x == "1":
                 ...
                 await borg.send_message(chat, "**i am offline Reeeeeeeeee.**")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
                 ...
                 await borg.send_message(chat, "`I dont care`")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
